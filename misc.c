...
double mindt_burn(double *,double ,double);
   ...
   for( j=1 ; j<Nr-1 ; ++j ){
      for( k=0 ; k<Nz ; ++k ){
	    ...
	    double dV = get_dV( xp , xm );
            ...
            double dt_temp = mindt( c->prim ,c->temp, w , xp , xm );
            
//           printf("%f   %f\n",c->prim[RHO],c->temp);
//	    double dt_burn=mindt_burn(c->prim,c->temp,c->cons[TAU]/dV);
            
           	    
//	    if( dt_temp>dt_burn/theDomain->theParList.CFL) dt_temp=dt_burn/theDomain->theParList.CFL;
   ...
   for( j=0 ; j<Nr ; ++j ){
      ...
      for( k=0 ; k<Nz ; ++k ){
         ...
         for( i=0 ; i<Np[jk] ; ++i ){
            ...
            double dV = get_dV( xp , xm );
            cons2prim( c->cons , c->prim ,&(c->temp), r , dV );
//            printf("e=%f\n",c->cons[TAU]/dV);	    
         }
      }
   }
...
void AMRsweep( struct domain * theDomain , struct cell ** swptr , int jk ){
   ...
   if( S>MaxShort ){
      ...
      cons2prim( sweep[iS].cons , sweep[iS].prim ,&(sweep[iS].temp), r , dV );
      //Shift Memory
   ...
   if( L>MaxLong ){
      ...
      *swptr = (struct cell *) realloc( sweep , Np[jk]*sizeof(struct cell) );
      ...
      cons2prim( sweep[iL].cons , sweep[iL].prim ,&(sweep[iL].temp), r , dV );

      xp[1] = phip;
      xm[1] = phi0;
      dV = get_dV( xp , xm );
      r  = get_moment_arm( xp , xm );
      cons2prim( sweep[iL+1].cons , sweep[iL+1].prim ,&(sweep[iL+1].temp),r , dV );
